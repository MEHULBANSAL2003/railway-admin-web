...
const EMPTY_ERRORS = {
  fullName: '', email: '', phoneNumber: '', role: '', department: '',
};
...
const validate = (form) => {
  ...
  if (!form.fullName.trim()) {
    errors.fullName = 'Full name is required.'; isValid = false;
  } else if (form.fullName.trim().length < 2) {
    errors.fullName = 'Name must be at least 2 characters.'; isValid = false;
  }
  ...
  if (!form.phoneNumber.trim()) {
    ...
    errors.phoneNumber = 'Enter a valid phone number (7–15 digits).'; isValid = false;
  }
...
const AddAdminModal = ({ open, onClose, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await AdminService.createNewAdmin(payload);
      const created = res?.data?.data;
      showSuccess(`Admin "${created?.name || form.fullName}" created successfully.`);
      handleClose();
      onSuccess?.();
    } catch (err) {
      ...
      <div className="aam-modal" onClick={e => e.stopPropagation()} role="dialog" aria-modal="true">
        ...
        <div className="aam-body">
          ...
          <div className="aam-field">
            <label className="aam-label">Full Name <span className="aam-required">*</span></label>
            <input
              className={`aam-input${errors.fullName ? ' aam-input--error' : ''}`}
              type="text"
              placeholder="e.g. Mehul Bansal"
              value={form.fullName}
              onChange={e => handleChange('fullName', e.target.value)}
              disabled={saving}
              autoFocus
            />
            {errors.fullName && <p className="aam-error">{errors.fullName}</p>}
          </div>
          ...
          <div className="aam-field">
            ...
            <div className="aam-phone-row">
              ...
              <input
                className={`aam-input aam-input--phone${errors.phoneNumber ? ' aam-input--error' : ''}`}
                type="tel"
            ...
            {errors.phoneNumber && <p className="aam-error">{errors.phoneNumber}</p>}
          </div>
